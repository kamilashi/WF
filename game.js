...
class Creature { //changes rules of the game
    ...
    scanPath(originX, originY, parent, cameFromDir) {
        const coordsToCkeck = new THREE.Vector2(originX, originY);
        //if ((this.allPathCoords.length > 25)||(this.paths.includes(table.slots[originX][originY]))) {
        //if(this.paths.includes(table.slots[originX][originY])){
        //    return;
        // } else 
        if ((this.allPathCoords.length > 25)){
                console.log("infinite cycle!")
                return;
            }else
        {
            if ((inRange(originX, -1, 5)) && (inRange(originY, -1, 5))) { //add cycle detection!!!
                if ((this.allPathCoords.includes(table.slots[originX][originY]))) {
                    //if(this.paths.includes(table.slots[originX][originY])){
                        console.log("Stop here! cycle prevention")
                        return;
                    }

                this.allPathCoords.push(table.slots[originX][originY]);
                parent[0] = table.slots[originX][originY]; //pass reference to slot
                let linkIndex = 0;
                console.log("parent: " + originX + ", " + originY);
                if (cameFromDir != 2) { linkIndex = this.checkSlot(originX - 1, originY, parent, 4, linkIndex); } //check upper
                if (cameFromDir != 1) { linkIndex = this.checkSlot(originX, originY + 1, parent, 3, linkIndex); } //check right
                if (cameFromDir != 4) { linkIndex = this.checkSlot(originX + 1, originY, parent, 2, linkIndex); } //check bottom
                if (cameFromDir != 3) { linkIndex = this.checkSlot(originX, originY - 1, parent, 1, linkIndex); } //check left
            }
        }
    }
...
AI = { //to be generalized later
    ...
    dealCards() {
        for (let i = 0; i < handCount; i++) { this.hand[i] = deck.pop(); }
        updateScreen(player);
    },
    drawCardsToFull() {
        let cardsToDraw = handCount - this.hand.length;
        for (let i = 0; i < cardsToDraw; i++) { this.hand[i] = deck.pop(); }
        updateScreen(player);
    },
    playTurn() {
